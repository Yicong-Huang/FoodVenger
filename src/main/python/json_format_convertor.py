...
restaurants.write('"id","name","image","addr"\n')

images = open('../../../data/csv/image.sql', 'w')
...
for i in range(20):
    with open('../../../data/page' + str(i) + '.txt') as file:
        ...
        for b in json_object['businesses']:
            print([str(index) , b['name'] , b['image_url'],"\""+ ', '.join(b['location']["display_address"])+"\"" + '\n'])
            index += 1
            restaurants.write(
                ','.join([str(index) , b['name'] , b['image_url'],"\""+ ', '.join(b['location']["display_address"])+"\"" + '\n']))
            images.write("Insert ")
